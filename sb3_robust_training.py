...
def main():
    ...
    try:
        for update in range(total_updates):
            ...
            # Manual logging every few updates
            if update % 5 == 0:  # Log every 5 updates
                current_timesteps = (update + 1) * steps_per_update
                progress = (update + 1) / total_updates
                
                # Get recent episode stats
                if len(model.ep_info_buffer) > 0:
                    recent_rewards = [ep['r'] for ep in model.ep_info_buffer[-50:]]
                    recent_lengths = [ep['l'] for ep in model.ep_info_buffer[-50:]]
                    ...
                    wandb.log({
                        ...
                        "train/episodes": len(model.ep_info_buffer)
                    })
                    ...
                    print(f"Update {update+1}/{total_updates} ({progress:.1%}) - Steps: {current_timesteps:,}")
